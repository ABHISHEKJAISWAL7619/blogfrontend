...
const Sidebar = (props) => {
  const [loading, setloading] = useState(false);
    ...
    const showModal = () => {
      ...
      //   let res = await axios.post('https://blogbackend-i274.onrender.com/posts/create',finalObj,{
      //     headers:{
    ...
    <div className='fixed left-0 top-[80px]'>
      <div>
          ...
          { loading===true?"loading...":  <form action="" className='flex flex-col'>
              ...
              <input value={formDetails.title} name='title' onChange={handleInputChanger} className='outline-none border  my-2 border-gray-400 py-2 px-4 rounded-md' type="text" />
              <label  className='mb-2' htmlFor="">Description</label>
              <textarea value={formDetails.description} name='description' onChange={handleInputChanger} className='outline-none border my-2 border-gray-400 py-2 px-4 rounded-md' id=""></textarea>
              <label className='mb-2 py-2 px-4 bg-amber-800 w-max  rounded-md hover:bg-amber-700' htmlFor="file">Image/Video</label>
              <input multiple onChange={handleFileChanger} id='file' hidden className='outline-none border my-2 border-gray-400 py-2 px-4 rounded-md' type="file" />
